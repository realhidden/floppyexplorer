...
function run(args, opts = {}) {
  ...
  return new Promise((resolve, reject) => {
    execFile(GW_BIN, args, { timeout: opts.timeout || 30000 }, (err, stdout, stderr) => {
      // gw writes all output to stderr and exits non-zero for non-fatal reasons
      // (e.g. GitHub update check rate-limited). Use combined output if available.
...
async function info() {
  ...
  for (const line of lines) {
    // Skip error/warning lines from gw (e.g. GitHub rate limit)
    if (/^\*\*|FATAL|ERROR/i.test(line.trim())) continue;
    const m = line.match(/^\s*(\w[\w\s]*?):\s+(.+)$/);
    if (m) {
      const key = m[1].trim().toLowerCase().replace(/\s+/g, '_');
      // Skip serial number for privacy
