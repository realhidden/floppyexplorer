...
// Read directory entries from a FAT12 disk (with VFAT LFN support)
function readFATDirectory(buf, disk, fs_info) {
  ...
  const entries = [];
  // Accumulate LFN fragments (they precede the SFN entry, in reverse order)
  let lfnParts = [];
  ...
  for (let i = 0; i < fs_info.rootEntries; i++) {
    ...
    if (attr === 0x0F) {
      const seq = flat[off] & 0x3F; // sequence number (1-based)
      const isLast = !!(flat[off] & 0x40); // last LFN fragment flag
      const chars = extractLFNChars(flat, off);

      if (isLast) lfnParts = [];
      // Store at the right position (seq 1 = first 13 chars, seq 2 = next 13, etc.)
    ...
    // Reconstruct LFN if we collected fragments
    let longName = null;
