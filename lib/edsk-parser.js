  ...
  for (const trk of disk.trackIndex) {
    ...
    for (const sec of sorted) {
      if (sec.dataOffset == null) {
        // Sector data is missing (truncated or unreadable)
...
// Get sectors with missing/invalid data (dataOffset is null)
function getMissingSectors(disk) {
  const missing = [];
  for (const trk of disk.trackIndex) {
    if (trk.missing) {
      for (let i = 0; i < (trk.sectorCount || 0); i++) {
        missing.push({ track: trk.track, side: trk.side, index: i });
      }
    } else {
      for (const sec of (trk.sectors || [])) {
        if (sec.dataOffset == null) {
          missing.push({ track: trk.track, side: trk.side, index: sec.index });
        }
      }
    }
  }
...
function parseTrack(buf, off, trkSize) {
  ...
  for (const sec of sectors) {
    ...
    if (dataOff + sec.size > trackEnd) {
      sec.truncated = true;
      sec.dataOffset = null; // Mark as unreadable so hex viewer shows dots
      break;
    }
    dataOff += sec.size;
  }
